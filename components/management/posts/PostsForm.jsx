...
const PostsForm = ({ type, post, setPost, submitting, handleSubmit, currentDataTime, setCurrentDataTime }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className='flex-center flex-col space-y-5'>
            ...
            <textarea placeholder="Treść" value={post.contents} onChange={(e) => setPost({ ...post, contents: e.target.value })} style={{ resize: "none" }} required className="text-[#353535] bg-gray-50 py-[10px] px-[10px] w-[750px] h-[450px] tracking-[0.5px] border-2 border-gray-300 rounded-[5px]" />
            <input type="text" placeholder="Nazwa zdjęcia z rozszerzeniem" name="img" value={post.img} onChange={(e) => setPost({ ...post, img: e.target.value })} className="text-[#353535] bg-gray-50 py-[10px] px-[10px] w-[750px] tracking-[0.5px] border-2 border-gray-300 rounded-[5px]" />
