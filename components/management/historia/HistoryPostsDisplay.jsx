...
const PostsDisplay = () => {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            try {
                const response = await fetch(`/api/history?page=${currentPage}&limit=${postsPerPage}`);
    ...
    const handleEdit = (post) => {
        router.push(`historia/edytuj?id=${post._id}`)
    }
    ...
    const handleDelete = async (post) => {
        ...
        if (hasConfirmed) {
            try {
                await fetch(`/api/history/${post._id.toString()}`, {
                    method: 'DELETE'
                });
